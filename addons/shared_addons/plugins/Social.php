...
class Plugin_Social extends Plugin
{
	...
	function facebook() {
		...
		$appId = ($this->attribute('appId')!='') ? $this->attribute('appId') : '454218078005711';

		$href = $this->attribute('href',current_url());

		$send = $this->attribute('send','true');

		$width = $this->attribute('width','300');

		$faces = $this->attribute('faces','true');

		$colorscheme = $this->attribute('colorscheme', 'light');

		$button = $this->attribute('button','like');

		$layout = $this->attribute('layout','button_count');
		...
		if ( ! in_array($button, array('like','follow','comments', 'send', 'share')))
		{
			$button = 'like';
		}
		...
		switch ($button) {
			case 'share':
				return '<a href="http://www.facebook.com/sharer.php?u=' . $href . '">Share on Facebook </a>';
				break;
